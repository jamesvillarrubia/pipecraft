...
function validateGitConfig(actionPath) {
  ...
  const hasGitConfig =
    content.includes('git config user.name') || content.includes('git config --global user.name')
...
function validateVersionFormat(actionPath) {
  ...
  if (content.includes('${{ inputs.version }}') || content.includes('${VERSION}')) {
    ...
    const hasStripPrefix =
      content.includes('VERSION="${VERSION#v}"') ||
    ...
    const hasVersionValidation =
      content.includes('=~ ^[0-9]+') || content.includes('version format')
        ...
        if (step.uses && step.uses.startsWith('./')) {
          if (!hasCheckout) {
            ...
            if (!actionHasCheckout) {
              errors.push(
                `Job '${jobName}' step ${i + 1}: Local action '${step.uses}' used before checkout`
              )
            }
          }
        }
