...
describe('LogAnalyzer', () => {
  ...
  it('should identify critical errors', () => {
    ...
    expect(analysis.criticalErrors.length).toBeGreaterThan(0)
    expect(
      analysis.criticalErrors.some(error => error.includes('Database connection refused'))
    ).toBe(true)
  })
})
...
describe('GitHubWorkflowDebugger', () => {
  ...
  it('should handle API request errors gracefully', async () => {
    ...
    await expect(workflowDebugger.getWorkflowRuns('test-workflow')).rejects.toThrow(
      'API request failed'
    )
  })
})
...
describe('WorkflowDebugOrchestrator', () => {
  ...
  it('should handle missing workflow runs', async () => {
    ...
    await expect(orchestrator.debugWorkflow('test-workflow')).rejects.toThrow(
